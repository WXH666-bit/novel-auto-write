...
def get_graph(
    ...
    nodes = db.scalars(
        ...
        .where(StoryGraphNode.project_id == project.id)
        .order_by(StoryGraphNode.created_at)
    ).all()
    edges = db.scalars(
        select(StoryGraphEdge)
        .where(StoryGraphEdge.project_id == project.id)
        .order_by(StoryGraphEdge.created_at)
    ).all()
    layout = db.scalar(
        select(StoryGraphLayout).where(StoryGraphLayout.project_id == project.id)
    )
...
def list_nodes(
    ...
    project = require_project(db, project_id, current_user)
    rows = db.scalars(
        select(StoryGraphNode).where(StoryGraphNode.project_id == project.id).order_by(StoryGraphNode.created_at)
    ).all()
...
def list_edges(
    ...
    project = require_project(db, project_id, current_user)
    rows = db.scalars(
        select(StoryGraphEdge).where(StoryGraphEdge.project_id == project.id).order_by(StoryGraphEdge.created_at)
    ).all()
...
def update_edge(
    ...
    if not values:
        return StoryGraphEdgeRead.model_validate(edge)
    if "source_node_id" in values or "target_node_id" in values:
...
def get_layout(
    ...
    project = require_project(db, project_id, current_user)
    layout = db.scalar(select(StoryGraphLayout).where(StoryGraphLayout.project_id == project.id))
    if layout is None:
...
def save_layout(
    ...
    layout = db.scalar(
        ...
        .where(StoryGraphLayout.project_id == project.id)
        .with_for_update()
    )
    if layout is None:
        if payload.expected_version not in (None, 0, 1):
            raise HTTPException(status_code=409, detail="图谱布局版本冲突")
        layout = StoryGraphLayout(project_id=project.id, layout_json=payload.layout_json)
        db.add(layout)
    else:
